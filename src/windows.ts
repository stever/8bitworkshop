...
import {FileData, WorkerError} from "./worker/defs_misc";
...
export class ProjectWindows {
    ...
    findWindowWithFilePrefix(filename: string): string {
        ...
        for (var fileid in this.id2createfn) {
            ...
            if (fileid.toLowerCase().endsWith('.h') ||
                fileid.toLowerCase().endsWith('.inc') ||
                fileid.toLowerCase().endsWith('.bas')) {
                continue;
            }

            if (getFilenameForPath(getFilenamePrefix(fileid)) == filename) {
                return fileid;
            }
        }
